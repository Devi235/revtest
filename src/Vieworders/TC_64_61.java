...
public class TC_64_61 extends Reuse{
	public ExtentReports report=new ExtentReports(ReportSavingPath+"\\Vieworders\\"+"TC_64_61.html");
	public ExtentTest logger =report.startTest("TC_64_61");
	String OrderNo1;
	@Test(priority=0)
	public void OrderNumberGet() throws Exception {
		...
		try{
		 ...
		 screenshot(ScreenshotSavingPath+"TC_55_01");
		 try{
				if(driver.findElement(By.xpath("(//*[text()='Close Order'])[1]")).isDisplayed()){
					driver.findElement(By.xpath("(//*[text()='Close Order'])[1]")).click();
					Thread.sleep(8000);
		 ...
		 logger.log(LogStatus.PASS,"Order placed",logger.addScreenCapture(ScreenshotSavingPath+"TC_55_01"));
	...
	public void TC_CTB() throws Exception {
		...
		//completed to billed
		 ...
		 waitexplicit(wait1);;Thread.sleep(4000);
		 screenshot(ScreenshotSavingPath+"TC_55_02");
		 String orderstatus=driver.findElement(By.xpath("((//*[text()='Order No.'])[1])//following::td[3]")).getText();
		 if(orderstatus.equals("Billed")){
			 logger.log(LogStatus.PASS,"Status change from Completed to Billed",logger.addScreenCapture(ScreenshotSavingPath+"TC_55_02")); 
		 }
		 else
		 {
			 logger.log(LogStatus.FAIL,"Status change from Completed to Billed",logger.addScreenCapture(ScreenshotSavingPath+"TC_55_02"));
		 }
		//billed to unbilled
		 try{
		...
		a.click(checkbox1).build().perform();Thread.sleep(1000);
		 ...
		 screenshot(ScreenshotSavingPath+"TC_55_03");
		 String orderstatus1=driver.findElement(By.xpath("((//*[text()='Order No.'])[1])//following::td[3]")).getText();
		 if(orderstatus1.equals("UnBilled")){
			 logger.log(LogStatus.PASS,"Status change from Billed to unBilled",logger.addScreenCapture(ScreenshotSavingPath+"TC_55_03")); 
		 }
		 else
		 {
			 logger.log(LogStatus.FAIL,"Status change from Billed to unBilled",logger.addScreenCapture(ScreenshotSavingPath+"TC_55_03"));
		 }}
		 catch(Exception e)
		 {
			 logger.log(LogStatus.FAIL,"Status change from Billed to unBilled",logger.addScreenCapture(ScreenshotSavingPath+"TC_55_03")); 
